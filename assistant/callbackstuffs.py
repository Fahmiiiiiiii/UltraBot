...
try:
    from pyultrabot.fns.gDrive import GDriveManager
except ImportError:
    GDriveManager = None
...
from pyultrabot.fns.helper import fast_download, progress
from pyultrabot.fns.tools import Carbon, async_searcher, get_paste, telegraph_client
from pyultrabot.startup.loader import Loader
...
_buttons = {
    "otvars": {
        "text": "Other Variables to set for @Teamultrabot:",
        "buttons": [
            [
    ...
    "oofdm": {
        "text": "About [Dual Mode](https://t.me/ultrabotUpdates/18)",
        "buttons": [
            [
...
async def update(eve):
    ...
    if heroku_api:
        ...
        execl(sys.executable, sys.executable, "-m", "pyultrabot")
    ...
    if not match:
        try:
            ...
            return await okk.edit(
                f"**• ultrabot Userbot •**{cli}", file=img, buttons=button
            )
        except Exception as er:
            LOGS.exception(er)
...
async def _(e):
    ...
    async with asst.conversation(e.sender_id) as conv:
        ...
        if GDrive._create_token_file(code=code.text):
            await conv.send_message(
                "`Success!\nYou are all set to use Google Drive with ultrabot Userbot.`",
...
async def _(e):
    if not e.is_private:
        return
    await e.edit(
        "Click Authorise and send the code.\n\nYou can use your own CLIENT ID and SECRET by [this](https://t.me/ultrabotUpdates/37)",
        buttons=[
            [
