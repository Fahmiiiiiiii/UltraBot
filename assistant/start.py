...
from pyultrabot._misc import SUDO_M, owner_and_sudos
from pyultrabot.dB.asst_fns import *
...
**ultrabot [v{ultrabot_version}](https://github.com/Fahmiiiiiiii/UltraBot), powered by @mailbintisukijem**
"""
...
async def own(event):
    msg = Owner_info_msg.format(
        mention=event.sender.mention, me=inline_mention(ultrabot_bot.me)
    )
    if custom_info:
...
async def ultrabot(event):
    ...
    if event.sender_id not in SUDO_M.fullsudos:
        ok = ""
        me = inline_mention(ultrabot_bot.me)
        ...
        if not udB.get_key("STARTMSG"):
            ...
            await event.reply(
                f"Hey there {mention}, this is ultrabot Assistant of {me}!\n\n{ok}",
