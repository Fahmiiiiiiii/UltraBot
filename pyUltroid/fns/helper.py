...
def un_plug(shortname):
    ...
    try:
        ...
        for client in [ultrabot_bot, asst]:
...
if run_as_module:
    ...
    async def safeinstall(event):
        ...
        if plug in HELP:
            ...
            for i in HELP[plug]:
                output += i
            output += "\n© @Teamultrabot"
            await eod(ok, f"✓ `ultrabot - Installed`: `{plug}` ✓\n\n{output}")
    ...
    async def heroku_logs(event):
        ...
        ok = app.get_log()
        with open("ultrabot-heroku.log", "w") as log:
            log.write(ok)
        await event.client.send_file(
            event.chat_id,
            file="ultrabot-heroku.log",
            thumb=ULTConfig.thumb,
            caption="**ultrabot Heroku Logs.**",
        )

        os.remove("ultrabot-heroku.log")
        await xx.delete()
    ...
    def gen_chlog(repo, diff):
        ...
        ch_log = tldr_log = ""
        ch = f"<b>ultrabot {ultrabot_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
        ch_tl = f"ultrabot {ultrabot_version} updates for {ac_br}:"
        d_form = "%d/%m/%y || %H:%M"
