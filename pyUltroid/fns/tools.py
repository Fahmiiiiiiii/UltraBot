...
async def get_chatbot_reply(message):
    chatbot_base = "https://kukiapi.xyz/api/apikey=ultrabotUSERBOT/ultrabot/{}/message={}"
    req_link = chatbot_base.format(
        ultrabot_bot.me.first_name or "ultrabot user",
        message,
    )
    try:
...
def telegraph_client():
    ...
    gd_name = ultrabot_bot.full_name
    short_name = gd_name[:30]
    profile_url = (
        f"https://t.me/{ultrabot_bot.me.username}"
        if ultrabot_bot.me.username
        else "https://t.me/Teamultrabot"
    )
    try:
        ...
        if "SHORT_NAME_TOO_LONG" in str(er):
            TelegraphClient.create_account(
                short_name="ultrabotuser", author_name=gd_name, author_url=profile_url
            )
        else:
            LOGS.exception(er)
            return
...
async def Carbon(
    ...
    **kwargs,
    ...
    if rayso:
        ...
        kwargs["title"] = kwargs.get("title", "ultrabot")
    else:
        kwargs["code"] = code
