...
from pyultrabot.exceptions import DependencyMissingError
...
async def randomchannel(
    tochat, channel, range1, range2, caption=None, client=ultrabot_bot
):
...
async def allcmds(event, telegraph):
    ...
    for z in LIST.keys():
        ...
        for zz in LIST[z]:
            txt += HNDLR + zz + "\n"
        txt += "\n\n"
    t = telegraph.create_page(title="ultrabot All Cmds", content=[txt])
    await eor(event, f"All ultrabot Cmds : [Click Here]({t['url']})", link_preview=False)
...
async def _insta_login():
    if "insta_creds" in ultrabot_bot._cache:
        return ultrabot_bot._cache["insta_creds"]
    ...
    if username and password:
        ...
        try:
            ...
            ultrabot_bot._cache.update({"insta_creds": cl})
...
async def get_insta_code(username, choice):
    ...
    async with asst.conversation(ultrabot_bot.uid, timeout=60 * 2) as conv:
