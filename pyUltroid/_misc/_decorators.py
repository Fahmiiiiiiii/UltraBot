...
from ..version import ultrabot_version as ult_ver
...
def ultrabot_cmd(
    pattern=None, manager=False, ultrabot_bot=ultrabot_bot, asst=asst, **kwargs
):
    ...
    def decor(dec):
        async def wrapp(ult):
            ...
            try:
                ...
                await ultrabot_bot.disconnect()
                await asyncio.sleep(fwerr.seconds + 10)
                await ultrabot_bot.connect()
                await asst.send_message(
                    udB.get_key("LOG_CHANNEL"),
                ...
                await asst.send_message(
                    ...
                    buttons=[
                        ...
                        Button.url(
                            "Repl",
                            "https://replit.com/@UltraBot/UltraBotStringSession",
                        ),
                    ],
                )
                sys.exit()
                ...
                ftext = "**ultrabot Client Error:** `Forward this to` @ultrabotSupportChat\n\n"
                ftext += "**Py-ultrabot Version:** `" + str(pyver)
                ftext += "`\n**ultrabot Version:** `" + str(ult_ver)
                ftext += "`\n**Telethon Version:** `" + str(telever)
                ftext += f"`\n**Hosted At:** `{HOSTED_ON}`\n\n"
                ftext += "--------START ultrabot CRASH LOG--------"
                ftext += "\n**Date:** `" + date
                ...
                if len(ftext) > 4096:
                    with BytesIO(ftext.encode()) as file:
                        ...
                        error_log = await asst.send_file(
                            udB.get_key("LOG_CHANNEL"),
                            file,
                            caption="**ultrabot Client Error:** `Forward this to` @ultrabotSupportChat\n\n",
                        )
                else:
        ...
        if _add_new:
            ...
            ultrabot_bot.add_event_handler(
                wrapp,
                NewMessage(
                    pattern=cmd,
                    incoming=True,
