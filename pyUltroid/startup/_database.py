...
class MongoDB(_BaseDatabase):
    def __init__(self, key, dbname="ultrabotDB"):
        self.dB = MongoClient(key, serverSelectionTimeoutMS=5000)
        self.db = self.dB[dbname]
        super().__init__()

    def __repr__(self):
        return f"<ultrabot.MonGoDB\n -total_keys: {len(self.keys())}\n>"
    ...
    def flushall(self):
        self.dB.drop_database("ultrabotDB")
        self._cache.clear()
        return True
...
# for SQL Implementation in ultrabot.
#
...
class SqlDB(_BaseDatabase):
    def __init__(self, url):
        ...
        try:
            ...
            self._cursor.execute(
                "CREATE TABLE IF NOT EXISTS ultrabot (ultrabotCli varchar(70))"
            )
    ...
    def usage(self):
        self._cursor.execute(
            "SELECT pg_size_pretty(pg_relation_size('ultrabot')) AS size"
        )
    ...
    def keys(self):
        self._cursor.execute(
            "SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name  = 'ultrabot'"
        )  # case sensitive
        data = self._cursor.fetchall()
    ...
    def get(self, variable):
        try:
            self._cursor.execute(f"SELECT {variable} FROM ultrabot")
    ...
    def set(self, key, value):
        try:
            self._cursor.execute(f"ALTER TABLE ultrabot DROP COLUMN IF EXISTS {key}")
        except (psycopg2.errors.UndefinedColumn, psycopg2.errors.SyntaxError):
            pass
        except BaseException as er:
            LOGS.exception(er)
        self._cache.update({key: value})
        self._cursor.execute(f"ALTER TABLE ultrabot ADD {key} TEXT")
        self._cursor.execute(f"INSERT INTO ultrabot ({key}) values (%s)", (str(value),))
        return True

    def delete(self, key):
        try:
            self._cursor.execute(f"ALTER TABLE ultrabot DROP COLUMN {key}")
        except psycopg2.errors.UndefinedColumn:
            return False
        return True
    ...
    def flushall(self):
        self._cache.clear()
        self._cursor.execute("DROP TABLE ultrabot")
        self._cursor.execute(
            "CREATE TABLE IF NOT EXISTS ultrabot (ultrabotCli varchar(70))"
        )
        return True
