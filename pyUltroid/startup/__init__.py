...
if run_as_module:
    ...
    from ..version import __version__ as __pyultrabot__
    from ..version import ultrabot_version

    file = f"ultrabot{sys.argv[6]}.log" if len(sys.argv) > 6 else "ultrabot.log"
    ...
    LOGS.info(f"py-ultrabot Version - {__pyultrabot__}")
    LOGS.info(f"Telethon Version - {__version__} [Layer: {LAYER}]")
    LOGS.info(f"ultrabot Version - {ultrabot_version} [{HOSTED_ON}]")
