...
async def autoupdate_local_database():
    from .. import asst, udB, ultrabot_bot, Var

    global db_url
    db_url = (
        udB.get_key("TGDB_URL") or Var.TGDB_URL or ultrabot_bot._cache.get("TGDB_URL")
    )
    if db_url:
        _split = db_url.split("/")
        _channel = _split[-2]
...
async def startup_stuff():
    ...
    if TZ and timezone:
        try:
            ...
            LOGS.critical(
                "Incorrect Timezone ,\nCheck Available Timezone From Here https://graph.org/ultrabot-06-18-2\nSo Time is Default UTC"
            )
            os.environ["TZ"] = "UTC"
            time.tzset()
...
async def autobot():
    from .. import udB, ultrabot_bot

    if udB.get_key("BOT_TOKEN"):
        return
    await ultrabot_bot.start()
    ...
    if who.username:
        ...
        username = "ultrabot_" + (str(who.id))[5:] + "_bot"
    bf = "@BotFather"
    await ultrabot_bot(UnblockRequest(bf))
    await ultrabot_bot.send_message(bf, "/cancel")
    await asyncio.sleep(1)
    await ultrabot_bot.send_message(bf, "/newbot")
    await asyncio.sleep(1)
    isdone = (await ultrabot_bot.get_messages(bf, limit=1))[0].text
    if isdone.startswith("That I cannot do.") or "20 bots" in isdone:
        ...
        sys.exit(1)
    await ultrabot_bot.send_message(bf, name)
    await asyncio.sleep(1)
    isdone = (await ultrabot_bot.get_messages(bf, limit=1))[0].text
    if not isdone.startswith("Good."):
        await ultrabot_bot.send_message(bf, "My Assistant Bot")
        await asyncio.sleep(1)
        isdone = (await ultrabot_bot.get_messages(bf, limit=1))[0].text
        if not isdone.startswith("Good."):
            LOGS.critical(
            ...
            sys.exit(1)
    await ultrabot_bot.send_message(bf, username)
    await asyncio.sleep(1)
    isdone = (await ultrabot_bot.get_messages(bf, limit=1))[0].text
    await ultrabot_bot.send_read_acknowledge("botfather")
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = "ultrabot_" + (str(who.id))[6:] + str(ran) + "_bot"
        await ultrabot_bot.send_message(bf, username)
        await asyncio.sleep(1)
        isdone = (await ultrabot_bot.get_messages(bf, limit=1))[0].text
    if isdone.startswith("Done!"):
        ...
        await enable_inline(ultrabot_bot, username)
        LOGS.info(
...
async def autopilot():
    from .. import asst, udB, ultrabot_bot

    channel = udB.get_key("LOG_CHANNEL")
    new_channel = None
    if channel:
        try:
            chat = await ultrabot_bot.get_entity(channel)
    ...
    if not channel:
        ...
        async def _save(exc):
            udB._cache["LOG_CHANNEL"] = ultrabot_bot.me.id
            await asst.send_message(
                ultrabot_bot.me.id, f"Failed to Create Log Channel due to {exc}.."
            )

        if ultrabot_bot._bot:
            msg_ = "'LOG_CHANNEL' not found! Add it in order to use 'BOTMODE'"
            LOGS.error(msg_)
            return await _save(msg_)
        LOGS.info("Creating a Log Channel for You!")
        try:
            r = await ultrabot_bot(
                CreateChannelRequest(
                    title="My ultrabot Logs",
                    about="My ultrabot Log Group\n\n Join @TeamUltrabot",
                    megagroup=True,
                ),
            )
    ...
    assistant = True
    try:
        await ultrabot_bot.get_permissions(int(channel), asst.me.username)
    except UserNotParticipantError:
        try:
            await ultrabot_bot(InviteToChannelRequest(int(channel), [asst.me.username]))
        except BaseException as er:
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        photo = await download_file(
            "https://i.postimg.cc/wBDvYZs2/20220910-112553.jpg", "channelphoto.jpg"
        )
        ll = await ultrabot_bot.upload_file(photo)
        try:
            await ultrabot_bot(
...
async def customize():
    ...
    try:
        ...
        if not ultrabot_bot.me.username:
            sir = ultrabot_bot.me.first_name
        else:
            sir = f"@{ultrabot_bot.me.username}"
        file = random.choice(
            [
                "https://i.postimg.cc/wBDvYZs2/20220910-112553.jpg",
                "https://i.postimg.cc/wBDvYZs2/20220910-112553.jpg",
        ...
        await asyncio.sleep(1)
        await ultrabot_bot.send_message("botfather", "/cancel")
        await asyncio.sleep(1)
        await ultrabot_bot.send_message("botfather", "/setuserpic")
        await asyncio.sleep(1)
        isdone = (await ultrabot_bot.get_messages("botfather", limit=1))[0].text
        if isdone.startswith("Invalid bot"):
            LOGS.info("Error while trying to customise assistant, skipping...")
            return
        await ultrabot_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await ultrabot_bot.send_file("botfather", file)
        await asyncio.sleep(2)
        await ultrabot_bot.send_message("botfather", "/setabouttext")
        await asyncio.sleep(1)
        await ultrabot_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await ultrabot_bot.send_message(
            "botfather", f"✨ Hello ✨!! I'm Assistant Bot of {sir}"
        )
        await asyncio.sleep(2)
        await ultrabot_bot.send_message("botfather", "/setdescription")
        await asyncio.sleep(1)
        await ultrabot_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await ultrabot_bot.send_message(
            "botfather",
            f"✨ Powerful ultrabot Assistant Bot ✨\n✨ Master ~ {sir} ✨\n\n✨ Powered By ~ @Teamultrabot ✨",
        )
        await asyncio.sleep(2)
...
async def plug(plugin_channels):
    ...
    for chat in plugin_channels:
        LOGS.info(f"{'•'*4} {chat}")
        try:
            async for x in ultrabot_bot.iter_messages(
                ...
                try:
                    ...
                    LOGS.info(f"ultrabot - PLUGIN_CHANNEL - ERROR - {plugin}")
                    LOGS.exception(e)
                    os.remove(plugin)
        except Exception as er:
            LOGS.exception(er)
...
async def ready():
    ...
    if not udB.get_key("INIT_DEPLOY"):  # Detailed Message at Initial Deploy
        MSG = """🎇 **Thanks for Deploying ultrabot Userbot!**
• Here, are the Some Basic stuff from, where you can Know, about its Usage."""
        PHOTO = "https://i.postimg.cc/wBDvYZs2/20220910-112553.jpg"
        BTTS = Button.inline("• Click to Start •", "initft_2")
        udB.set_key("INIT_DEPLOY", "Done")
    else:
        MSG = f"**ultrabot has been deployed!**\n➖➖➖➖➖➖➖➖➖➖\n**UserMode**: {inline_mention(ultrabot_bot.me)}\n**Assistant**: @{asst.me.username}\n➖➖➖➖➖➖➖➖➖➖\n**Support**: @Teamultrabot\n➖➖➖➖➖➖➖➖➖➖"
        BTTS, PHOTO = None, None
        prev_spam = udB.get_key("LAST_UPDATE_LOG_SPAM")
        if prev_spam:
            try:
                await ultrabot_bot.delete_messages(chat_id, int(prev_spam))
            except Exception as E:
    ...
    try:
        ...
        try:
            await (await ultrabot_bot.send_message(chat_id, str(e))).delete()
            spam_sent = await asst.send_message(chat_id, MSG, file=PHOTO, buttons=BTTS)
        ...
        LOGS.info(el)
        try:
            spam_sent = await ultrabot_bot.send_message(chat_id, MSG)
    ...
    try:
        updts = await async_searcher(
            "https://ultrabot-api.vercel.app/announcements", post=True, re_json=True
        )
        for upt in updts:
            key = list(upt.keys())[0]
            if key not in get_:
...
async def WasItRestart(udb):
    ...
    if not key:
        return
    from .. import asst, ultrabot_bot

    try:
        data = key.split("_")
        who = asst if data[0] == "bot" else ultrabot_bot
...
async def enable_inline(ultrabot_bot, username):
    bf = "BotFather"
    await ultrabot_bot.send_message(bf, "/setinline")
    await asyncio.sleep(1)
    await ultrabot_bot.send_message(bf, f"@{username}")
    await asyncio.sleep(1)
    await ultrabot_bot.send_message(bf, "Search")
    await ultrabot_bot.send_read_acknowledge(bf)
