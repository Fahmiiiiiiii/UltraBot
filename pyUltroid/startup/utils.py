...
def load_addons(plugin_name):
    ...
    if base_name.startswith("__"):
        return
    from .. import HNDLR, LOGS, asst, udB, ultrabot_bot
    from .._misc import _supporter as xxx
    from pyultrabot import fns
    from .._misc._assistant import asst_cmd, callback, in_pattern
    from .._misc._decorators import ultrabot_cmd
    ...
    mod.asst = asst
    mod.tgbot = asst
    mod.ultrabot_bot = ultrabot_bot
    mod.ub = ultrabot_bot
    mod.bot = ultrabot_bot
    mod.ultrabot = ultrabot_bot
    mod.borg = ultrabot_bot
    mod.telebot = ultrabot_bot
    mod.jarvis = ultrabot_bot
    mod.friday = ultrabot_bot
    mod.eod = eod
    mod.edit_delete = eod
    mod.LOGS = LOGS
    ...
    mod.ultrabot_cmd = ultrabot_cmd
    mod.on_cmd = ultrabot_cmd
    ...
    modules["pyultrabot.functions"] = fns
