...
from pyultrabot._misc._assistant import callback, in_pattern
from pyultrabot.dB._core import HELP, LIST
from pyultrabot.fns.helper import gen_chlog, time_formatter, updater
from pyultrabot.fns.misc import split_list
...
SUP_BUTTONS = [
    [
        Button.url("• Repo •", url="https://github.com/Fahmiiiiiiii/UltraBot"),
        Button.url("• Support •", url="t.me/UltraBotSupport"),
    ],
]
...
async def inline_alive(o):
    TLINK = inline_pic() or "https://i.postimg.cc/wBDvYZs2/20220910-112553.jpg"
    MSG = "• **ultrabot Userbot •**"
    WEB0 = InputWebDocument(
        "https://i.postimg.cc/wBDvYZs2/20220910-112553.jpg", 0, "image/jpg", []
    )
    RES = [
...
async def inline_handler(event):
    ...
    if inline_pic():
        ...
        result = await event.builder.article(
            title="ultrabot Help Menu", text=text, buttons=_main_help_menu
        )
...
async def _(event):
    ...
    if len(changelog_str) > 1024:
        ...
        with open("ultrabot_updates.txt", "w+") as file:
            file.write(tl_chnglog)
        await event.edit(
            get_string("upd_5"),
            file="ultrabot_updates.txt",
            buttons=[
                [Button.inline("• Uᴘᴅᴀᴛᴇ Nᴏᴡ •", data="updatenow")],
                [Button.inline("« Bᴀᴄᴋ", data="ownr")],
            ],
        )
        remove("ultrabot_updates.txt")
    else:
        await event.edit(
            changelog_str,
            buttons=[
...
async def ibuild(e):
    ...
    if pic:
        try:
            ...
            LOGS.exception(er)
    result = [
        await builder.article("ultrabot Op", text=txt, link_preview=False, buttons=btn)
    ]
