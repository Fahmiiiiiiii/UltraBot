...
from pyultrabot.fns.admins import ban_time
...
@ultrabot_cmd(pattern="pin$", manager=True, require="pin_messages", fullsudo=True)
async def pin(msg):
    if not msg.is_reply:
...
@ultrabot_cmd(pattern="purge( (.*)|$)", manager=True, require="delete_messages")
...
@ultrabot_cmd(pattern="pinned", manager=True, groups_only=True)
