...
from pyultrabot import _ignore_eval
...
try:
    ...
    from pyultrabot.fns.tools import safe_load
try:
...
@ultrabot_cmd(pattern="bash", fullsudo=True, only_devs=True)
async def _(event):
...
@ultrabot_cmd(pattern="eval", fullsudo=True, only_devs=True)
async def _(event):
    try:
        cmd = event.text.split(maxsplit=1)[1]
    except IndexError:
    ...
    if any(item in cmd for item in KEEP_SAFE().All) and (
        not (event.out or event.sender_id == ultrabot_bot.uid)
    ):
...
@ultrabot_cmd(pattern="cpp", only_devs=True)
async def doie(e):
    ...
    open("cpp-ultrabot.cpp", "w").write(match)
    m = await bash("g++ -o Cppultrabot cpp-ultrabot.cpp")
    o_cpp = f"• **Eval-Cpp**\n`{match}`"
    if m[1]:
        o_cpp += f"\n\n**• Error :**\n`{m[1]}`"
        if len(o_cpp) > 3000:
            os.remove("cpp-ultrabot.cpp")
            if os.path.exists("Cppultrabot"):
                os.remove("Cppultrabot")
