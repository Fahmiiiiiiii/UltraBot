...
from . import LOGS, get_help, get_string, udB, ultrabot_bot, ultrabot_cmd

__doc__ = get_help("help_autopic")


@ultrabot_cmd(pattern="autopic( (.*)|$)")
async def autopic(e):
...
if search := udB.get_key("AUTOPIC"):
    ...
    async def autopic_func():
        ...
        file = await ultrabot_bot.upload_file(img)
        await ultrabot_bot(UploadProfilePhotoRequest(file))
        shuffle(ok)
