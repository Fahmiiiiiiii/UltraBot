...
from pyultrabot.dB.filter_db import add_filter, get_filter, list_filter, rem_filter
...
@ultrabot_cmd(pattern="addfilter( (.*)|$)")
async def af(e):
    ...
    ultrabot_bot.add_handler(filter_func, events.NewMessage())


@ultrabot_cmd(pattern="remfilter( (.*)|$)")
async def rf(e):
    wrd = (e.pattern_match.group(1).strip()).lower()
    chat = e.chat_id
    if not wrd:
...
@ultrabot_cmd(pattern="listfilter$")
async def lsnote(e):
