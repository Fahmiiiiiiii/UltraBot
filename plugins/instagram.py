...
from pyultrabot.fns.helper import numerize
from pyultrabot.fns.misc import create_instagram_client
...
@ultrabot_cmd(pattern="instadata( (.*)|$)")
async def soon_(e):
    cl = await create_instagram_client(e)
    if not cl:
        return
    ...
    await e.reply(
        ...
        file=photo,
        force_document=True,
        attributes=[DocumentAttributeFilename("Instaultrabot.jpg")],
    )
    await ew.delete()


@ultrabot_cmd(pattern="(instaul|reels|igtv)( (.*)|$)")
