...
from pyultrabot.dB import DEVLIST
from pyultrabot.dB.logusers_db import *
from pyultrabot.dB.pmpermit_db import *
...
if udB.get_key("PMSETTING"):
    if udB.get_key("AUTOAPPROVE"):
        ...
        async def autoappr(e):
            ...
            try:
                await ultrabot_bot.edit_folder(miss.id, folder=0)
    ...
    async def permitpm(event):
        ...
        if not is_approved(user.id) and event.text != UND:
            if Redis("MOVE_ARCHIVE"):
                try:
                    await ultrabot_bot.edit_folder(user.id, folder=1)
            ...
            if user.id in LASTMSG:
                ...
                if event.text != prevmsg:
                    ...
                    if inline_pm:
                        results = await ultrabot_bot.inline_query(
                        ...
                        _to_delete[user.id] = await ultrabot_bot.send_file(
                            user.id,
                            PMPIC,
                            caption=message_,
                        )
                    else:
                        _to_delete[user.id] = await ultrabot_bot.send_message(
                            user.id, message_
                        )
            ...
            if COUNT_PM[user.id] >= WARNS:
                ...
                await ultrabot_bot(BlockRequest(user.id))
                await ultrabot_bot(ReportSpamRequest(peer=user.id))
                await asst.edit_message(
                    int(udB.get_key("LOG_CHANNEL")),
                    _not_approved[user.id],
    ...
    @ultrabot_cmd(pattern="(start|stop|clear)archive$", fullsudo=True)
    async def _(e):
    ...
    @ultrabot_cmd(pattern="(a|approve)(?: |$)", fullsudo=True)
    ...
    @ultrabot_cmd(pattern="(da|disapprove)(?: |$)", fullsudo=True)
...
@ultrabot_cmd(pattern="block( (.*)|$)", fullsudo=True)
...
@ultrabot_cmd(pattern="unblock( (.*)|$)", fullsudo=True)
...
@ultrabot_cmd(pattern="listapproved$", owner=True)
async def list_approved(event):
    ...
    for i in all:
        try:
            name = get_display_name(await ultrabot_bot.get_entity(i))
        except BaseException:
            name = ""
...
async def apr_in(event):
    ...
    if not is_approved(uid):
        approve_user(uid)
        try:
            await ultrabot_bot.edit_folder(uid, folder=0)
        except BaseException:
            pass
        try:
            user = await ultrabot_bot.get_entity(uid)
        except BaseException:
...
async def blck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    try:
        await ultrabot_bot(BlockRequest(uid))
    except BaseException:
        pass
    try:
        user = await ultrabot_bot.get_entity(uid)
...
async def unblck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    try:
        await ultrabot_bot(UnblockRequest(uid))
    except BaseException:
        pass
    try:
        user = await ultrabot_bot.get_entity(uid)
...
async def ytfuxist(e):
    try:
        ...
        await ultrabot_bot.delete_messages(e.chat_id, e.id)
...
async def in_pm_ans(event):
    ...
    await event.answer(res, switch_pm="• ultrabot •", switch_pm_param="start")


@callback(re.compile("admin_only(.*)"), from_users=[ultrabot_bot.uid])
