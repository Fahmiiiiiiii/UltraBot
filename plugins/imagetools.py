...
@ultrabot_cmd(pattern="(grey|blur|negative|danger|mirror|quad|sketch|flip|toon)$")
async def ult_tools(event):
    ...
    ult = cv2.imread(file)
    if match == "grey":
        ultrabot = cv2.cvtColor(ult, cv2.COLOR_BGR2GRAY)
    elif match == "blur":
        ultrabot = cv2.GaussianBlur(ult, (35, 35), 0)
    elif match == "negative":
        ultrabot = cv2.bitwise_not(ult)
    elif match == "danger":
        dan = cv2.cvtColor(ult, cv2.COLOR_BGR2RGB)
        ultrabot = cv2.cvtColor(dan, cv2.COLOR_HSV2BGR)
    elif match == "mirror":
        ish = cv2.flip(ult, 1)
        ultrabot = cv2.hconcat([ult, ish])
    elif match == "flip":
        trn = cv2.flip(ult, 1)
        ish = cv2.rotate(trn, cv2.ROTATE_180)
        ultrabot = cv2.vconcat([ult, ish])
    elif match == "quad":
        ult = cv2.imread(file)
        roid = cv2.flip(ult, 1)
        ...
        ultrabot = cv2.vconcat([mici, trn])
    elif match == "sketch":
        gray_image = cv2.cvtColor(ult, cv2.COLOR_BGR2GRAY)
        inverted_gray_image = 255 - gray_image
        blurred_img = cv2.GaussianBlur(inverted_gray_image, (21, 21), 0)
        inverted_blurred_img = 255 - blurred_img
        ultrabot = cv2.divide(gray_image, inverted_blurred_img, scale=256.0)
    elif match == "toon":
        ...
        ultrabot = ish.reshape(ult.shape)
    cv2.imwrite("ult.jpg", ultrabot)
...
@ultrabot_cmd(pattern="pixelator( (.*)|$)")
async def pixelator(event):
    ...
    await msg.respond("• Pixelated by ultrabot", file="output.jpg")
    await msg.delete()
