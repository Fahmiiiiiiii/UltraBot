...
from pyultrabot.fns.google_image import googleimagesdownload
from pyultrabot.fns.tools import metadata
...
async def hbd(event):
    ...
    if event.reply_to_msg_id:
        ...
        name = ultrabot_bot.me.first_name
...
@ultrabot_cmd(pattern="q( (.*)|$)", manager=True, allow_pm=True)
async def quott_(event):
    ...
    message = await reply.reply("Quotly by ultrabot", file=file)
    os.remove(file)
    await msg.delete()
    return message
