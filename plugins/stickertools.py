...
async def pack_kangish(_):
    ...
    _packname = cmdtext or f"ultrabot Kang Pack By {_.sender_id}"
    typee = None
    if not local:
...
async def hehe(args):
    ultrabot_bot = args.client
    xx = await args.eor(get_string("com_1"))
    user = ultrabot_bot.me
    ...
    if message.photo:
        photo = io.BytesIO()
        photo = await ultrabot_bot.download_media(message.photo, photo)
    elif message.file and "image" in message.file.mime_type.split("/"):
        photo = io.BytesIO()
        await ultrabot_bot.download_file(message.media.document, photo)
        if (
        ...
        await ultrabot_bot.download_file(
    ...
    if photo:
        ...
        if not emoji:
            emoji = "🏵"
        if len(splat) == 3:
            pack = splat[2]  # User sent ultrabot_both
            emoji = splat[1]
        elif len(splat) == 2:
        ...
        if (
            ...
            async with ultrabot_bot.conversation("@Stickers") as conv:
                try:
                    await conv.send_message("/addsticker")
                except YouBlockedUserError:
                    LOGS.info("Unblocking @Stickers for kang...")
                    await ultrabot_bot(functions.contacts.UnblockRequest("stickers"))
                    await conv.send_message("/addsticker")
                ...
                await conv.get_response()
                await ultrabot_bot.send_read_acknowledge(conv.chat_id)
        else:
...
async def ultdestroy(event):
    ...
    await event.client.download_media(ult, "ultrabot.tgs")
    xx = await event.eor(get_string("com_1"))
    await bash("lottie_convert.py ultrabot.tgs json.json")
    ...
    file = await con.animated_sticker("json.json", "ultrabot.tgs")
    if file:
        await event.client.send_file(
            event.chat_id,
            file="ultrabot.tgs",
...
async def ultiny(event):
    ...
    im1 = Image.open("resources/extras/ultrabot_blank.png")
    if ik.endswith(".tgs"):
