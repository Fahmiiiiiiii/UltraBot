...
from pyultrabot.dB.night_db import *

from . import get_string, udB, ultrabot_bot, ultrabot_cmd


@ultrabot_cmd(pattern="nmtime( (.*)|$)")
...
@ultrabot_cmd(pattern="addnm( (.*)|$)")
async def add_grp(e):
    if pat := e.pattern_match.group(1).strip():
        try:
            add_night((await ultrabot_bot.get_entity(pat)).id)
...
@ultrabot_cmd(pattern="remnm( (.*)|$)")
async def rem_grp(e):
    if pat := e.pattern_match.group(1).strip():
        try:
            rem_night((await ultrabot_bot.get_entity(pat)).id)
...
@ultrabot_cmd(pattern="listnm$")
async def rem_grp(e):
    chats = night_grps()
    name = "NightMode Groups Are-:\n\n"
    for x in chats:
        try:
            ok = await ultrabot_bot.get_entity(x)
...
async def open_grp():
    ...
    for chat in chats:
        try:
            ...
            await ultrabot_bot.send_message(chat, "**NightMode Off**\n\nGroup Opened 🥳.")
