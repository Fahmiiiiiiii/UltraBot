...
from pyultrabot.fns.tools import metadata, translate
...
from . import inline_mention, is_url_ok, mediainfo, ultrabot_cmd


@ultrabot_cmd(pattern="tr( (.*)|$)", manager=True)
async def _(event):
...
@ultrabot_cmd(pattern="bots( (.*)|$)", groups_only=True, manager=True)
async def _(ult):
...
async def magic(event):
    ...
    data = await async_searcher(
        "https://tiny.ultrabot.tech/api/new",
        data=data,
        post=True,
        re_json=True,
    )
