...
from pyultrabot._misc import sudoers
from pyultrabot.dB.snips_db import add_snip, get_snips, list_snip, rem_snip
from pyultrabot.fns.tools import create_tl_btn, format_btn, get_msg_button

from . import events, get_string, mediainfo, udB, ultrabot_bot, ultrabot_cmd
from ._inline import something


@ultrabot_cmd(pattern="addsnip( (.*)|$)")
async def an(e):
    ...
    ultrabot_bot.add_handler(add_snips, events.NewMessage())


@ultrabot_cmd(pattern="remsnip( (.*)|$)")
async def rs(e):
    wrd = (e.pattern_match.group(1).strip()).lower()
    if not wrd:
    ...
    for z in xx:
        if k := get_snips(z):
            ...
            if rep:
                ...
                await ultrabot_bot.send_message(e.chat_id, msg, file=media)
