...
from pyultrabot.dB import autoban_db, dnd_db
from pyultrabot.fns.admins import get_update_linked_chat

from . import LOGS, asst, get_string, inline_mention, ultrabot_bot, ultrabot_cmd
...
@ultrabot_cmd(pattern="cban$", admins_only=True)
...
@ultrabot_cmd(pattern="(list|add|rem)wl( (.*)|$)")
...
if dnd_db.get_dnd_chats():
    ultrabot_bot.add_handler(dnd_func, events.ChatAction(func=lambda x: x.user_joined))
