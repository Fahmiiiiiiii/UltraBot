...
from pyultrabot._misc._assistant import asst_cmd
from pyultrabot.dB.gban_mute_db import is_gbanned
from pyultrabot.fns.tools import get_chat_and_msgid
...
@ultrabot_cmd(pattern="kickme$", fullsudo=True)
async def leave(ult):
...
@ultrabot_cmd(pattern="paste( (.*)|$)", manager=True, allow_all=True)
async def _(event):
    try:
...
async def telegraphcmd(event):
    xx = await event.eor(get_string("com_1"))
    match = event.pattern_match.group(1).strip() or "ultrabot"
    reply = await event.get_reply_message()
    if not reply:
...
@ultrabot_cmd(pattern="suggest( (.*)|$)", manager=True)
...
async def get_restriced_msg(event):
    ...
    if not (chat and msg):
        return await event.eor(
            f"{get_string('gms_1')}!\nEg: `https://t.me/TeamUltrabot/3 or `https://t.me/c/1313492028/3`"
        )
    try:
