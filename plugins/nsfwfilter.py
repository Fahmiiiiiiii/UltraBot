...
from pyultrabot.dB.nsfw_db import is_nsfw, nsfw_chat, rem_nsfw

from . import HNDLR, async_searcher, eor, events, udB, ultrabot_bot, ultrabot_cmd


@ultrabot_cmd(pattern="addnsfw( (.*)|$)", admins_only=True)
async def addnsfw(e):
    ...
    ultrabot_bot.add_handler(nsfw_check, events.NewMessage(incoming=True))
    await e.eor("Added This Chat To Nsfw Filter")


@ultrabot_cmd(pattern="remnsfw", admins_only=True)
async def remnsfw(e):
    rem_nsfw(e.chat_id)
        ...
        if nsfw:
            ...
            if NWARN.get(e.sender_id):
                ...
                if count < 3:
                    ...
                    return await ultrabot_bot.send_message(
                        chat,
                ...
                if "mute" in action:
                    ...
                    try:
                        await ultrabot_bot.kick_participant(chat, e.sender_id)
                        await ultrabot_bot.send_message(
                            chat,
                            f"NSFW Warn 3/3 to [{e.sender.first_name}](tg://user?id={e.sender_id})\n\n**Action Taken** : {action}",
                        )
