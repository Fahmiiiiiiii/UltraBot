...
from pyultrabot.dB.blacklist_db import (
    add_blacklist,
    get_blacklist,
    list_blacklist,
    rem_blacklist,
...
from . import events, get_string, udB, ultrabot_bot, ultrabot_cmd


@ultrabot_cmd(pattern="blacklist( (.*)|$)", admins_only=True)
async def af(e):
    ...
    ultrabot_bot.add_handler(blacklist, events.NewMessage(incoming=True))
    await e.eor(get_string("blk_2").format(wrd))


@ultrabot_cmd(pattern="remblacklist( (.*)|$)", admins_only=True)
async def rf(e):
    wrd = e.pattern_match.group(1).strip()
    chat = e.chat_id
    if not wrd:
...
@ultrabot_cmd(pattern="listblacklist$", admins_only=True)
async def lsnote(e):
